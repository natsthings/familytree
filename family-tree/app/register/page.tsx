    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-8">
          ...
          <p className="text-sm text-[var(--parchment-dim)] font-body italic mb-6">You'll be the root of your family tree.</p>
          {error && (
            <div className="mb-4 p-3 bg-red-950/50 border border-red-800/50 rounded-lg text-red-300 text-sm font-body">{error}</div>
          )}
          <form onSubmit={handleRegister} className="space-y-4">
            <div>
              <label className="block text-xs font-mono text-[var(--parchment-dim)] uppercase tracking-widest mb-1.5">Your Name</label>
          ...
          <p className="text-center text-sm font-body text-[var(--parchment-dim)] mt-6">
            ...
            <Link href="/login" className="text-[var(--gold)] hover:text-[var(--gold-bright)] transition-colors">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
